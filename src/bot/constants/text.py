...
FUND_PROJECTS = {
    'part_1': (
        ...
        'гипотезы и принимаем те, которые сработали, а от тех, '
        'которые не дали нужный результат, отказываемся. '
        'Сейчас у Фонда есть несколько действующих проектов, рассказываю! 🤩'
    ),
    'part_2': (
        'В Фонде реализуется программа поддержки социальных предпринимателей, '
        'которая включает в себя обучающие мероприятия, консультации, '
        'обмен опытом и нетворкинг для социальных предпринимателей и НКО '
        'в регионах, которые работают в сфере сбора '
        'и перераспределения текстиля.'
    ),
    'part_3': (
        ...
        'себя в дизайне и творчестве и узнать больше '
    ...
    'part_5': (
        'О реализации этих проектов и не только, мы рассказываем в '
        'годовых отчетах Фонда. Прислать тебе ссылку?'
    )
}

ANNUAL_REPORTS = (
    f'*Почитаи наши [годовые отчеты]({ANNUAL_REPORTS_LINK})\.* '
