...
async def rules_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/1:5/')
    rules_keyboard = [
        [
...
async def communication_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/6:9/')
...
async def workshop_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/10:11/')
    workshop_keyboard = [
        [
            InlineKeyboardButton(
...
async def kitchen_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/12:13/')
    kitchen_keyboard = [
        [
            InlineKeyboardButton(
...
async def separate_collection_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/14:15/')
...
async def regular_meetings_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/16:17/')
...
async def in_communication_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/18:19/')
...
async def out_communication_markup():
    messages = await get_django_json(f'{api_root}rules_keyboards/20:22/')
    message_link = await get_django_json(f'{api_root}rules_text/14/')
